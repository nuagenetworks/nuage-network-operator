...
func (r *NuageCNIConfigReconciler) GetClusterNetworkInfo() (*operv1.ClusterNetworkConfigDefinition, error) {
...
func (r *NuageCNIConfigReconciler) UpdateClusterNetworkStatus(c *operv1.ClusterNetworkConfigDefinition) error {
	...
	nsn := types.NamespacedName{Name: clusterConfig.GetName()}

	tmp := &configv1.Network{}

	err := r.Client.Get(context.TODO(), nsn, tmp)
	if err != nil && strings.Contains(err.Error(), "not found") {
		err = r.Client.Create(context.TODO(), clusterConfig)
		if err != nil {
			return err
		}
		return nil
	...
	tmp.Status = clusterConfig.Status
